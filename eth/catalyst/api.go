...
func (api *ConsensusAPI) forkchoiceUpdated(update engine.ForkchoiceStateV1, payloadAttributes *engine.PayloadAttributes) (engine.ForkChoiceResponse, error) {
	...
	} else if !api.eth.BlockChain().Config().EnableL2GasLimitApi { // minor Engine API divergence: allow proposers to reorg their own chain
		// <specular modification/>
...
func (api *ConsensusAPI) heartbeat() {
	// <specular modification>
	if api.eth.BlockChain().Config().EnableL2GasLimitApi { // don't start the api heartbeat, there is no transition
		return
	}
	// <specular modification/>
